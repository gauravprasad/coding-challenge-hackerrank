...
Filter positions in a list (Hackerrank Scala Solution)
...
object FilterPositionsInList {
  def f(arr:List[Int]):List[Int]=arr.zipWithIndex.filter(x=>x._2%2==1).map(x=>x._1).toList
  def main(args: Array[String]) {
        ...
        for(i<- 0 to 10)
        {
          list=list:::List(io.StdIn.readInt())
        }
